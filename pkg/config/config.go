...
// 		specific file. E.g. refer /configs/* directory.
//
// Usage:
...
// is different from build time path as runtime.Caller() evaluates
// only at build time
...
func (c *Config) loadByConfigName(configName string, config interface{}) error {
	c.viper.SetEnvPrefix(strings.ToUpper("go-rest-webserver"))
