...
func replaceGormCallbacks(db *DB) {
	...
	// In this case the callback written in models won't come into picture.
...
func updateTimestampOnCreate(scope *gorm.Scope) {
	if createdAtField, ok := scope.FieldByName(ColumnCreatedAt); ok {
		// If its blank then only we update it with current timestamp
		if createdAtField.IsBlank {
			_ = scope.SetColumn(ColumnCreatedAt, time.Now().Unix())
		}
	}
...
func updateTimestampOnUpdate(scope *gorm.Scope) {
	if _, ok := scope.FieldByName(ColumnUpdatedAt); ok {
		_ = scope.SetColumn(ColumnUpdatedAt, time.Now().Unix())
	}
}
...
func updateTimestampOnDelete(scope *gorm.Scope) {
	...
	if !scope.HasError() {
		...
		deletedAtField, hasDeletedAtField := scope.FieldByName(ColumnDeletedAt)
