...
import (
	"encoding/json"
	"go-rest-webserver-template/pkg/errorresponse"
	"strings"
)
...
func (e Error) Status() int {
	if strings.HasPrefix(e.public.Code, "BAD_REQUEST") {
		if e.class.code == AuthorizationError {
			return 401
		} else {
			return 400
		}
	} else if strings.HasPrefix(e.public.Code, "SERVER_ERROR") {
		return 500
	}
...
func (e Error) Public() IPublic {
	if e.class.code == ValidationFailure && e.internal != nil && e.internal.cause != nil {
		e.public.Description = e.internal.cause.Error()
	}
